...
import { Heart, Search, SlidersHorizontal, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function BuyPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/buy-properties')
      ...
      .catch(err => alert(`Could not load properties.\nError: ${err.message}`));
  }, []);
  ...
  useEffect(() => {
    socket.on('new-property', (newProp) => {
      const standardized = {
        ...
        desc: newProp.desc || 'No description available.',
      };
...
const openContact = () => {
  ...
  setContactPropId(String(selectedProp?._id || ""));
  setShowContact(true);
};
    ...
    <>
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .card-prop:hover {
          ...
          box-shadow: 0 20px 40px rgba(28,43,26,0.12);
      ...
      <div style={{ minHeight: '100vh', background: 'var(--cream)', color: 'var(--charcoal)' }}>
        ...
        <section style={{ position: 'relative', height: '320px', overflow: 'hidden' }}>
          <img src="/him.jpg" alt="Hero" style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.8)', margin: 0, fontWeight: 300 }}>
              Curated properties across India's finest cities
            </p>
          </div>
        </section>
        ...
        <div id="listings" style={{ display: 'flex', minHeight: 'calc(100vh - 380px)' }}>
          ...
          {sidebarOpen && (
            <div className="overlay-sidebar" onClick={() => setSidebarOpen(false)} />
          )}
          <aside style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              ...
              {activeFiltersCount > 0 && (
                <button onClick={() => {
                  ...
                  background: 'none', border: 'none', color: 'var(--bark)',
                  fontSize: '12px', cursor: 'pointer', fontWeight: 500, textDecoration: 'underline'
                }}>Clear all</button>
              )}
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  type="text"
                  placeholder="City, title, location..."
                  value={searchInput}
                ...
                <button onClick={applyFilters} style={{
                  ...
                  background: 'none', border: 'none', cursor: 'pointer', color: 'var(--moss)',
          ...
          <main style={{ flex: 1, padding: '28px 24px', minWidth: 0 }}>
            ...
            <div style={{
              ...
              <div>
                <span className="font-display" style={{ fontSize: '28px', fontWeight: 600, color: 'var(--forest)' }}>
                  {filtered.length}
                </span>
                ...
                <article key={p.id} className="card-prop">
                  <div style={{ position: 'relative', height: '200px', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'absolute', top: '12px', left: '12px', display: 'flex', gap: '6px' }}>
                      <span style={{
                        ...
                        color: 'var(--forest)', letterSpacing: '0.04em'
                      }}>{p.type}</span>
                      <span style={{
                        background: 'rgba(255,255,255,0.92)', borderRadius: '6px',
                        padding: '3px 10px', fontSize: '11px', fontWeight: 600, color: 'var(--charcoal)'
                      }}>{p.beds} BHK</span>
                    </div>
                  ...
                  <div style={{ padding: '16px' }}>
                    <h3 className="font-display" style={{
                      fontSize: '18px', fontWeight: 600, margin: '0 0 4px',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span className="font-display" style={{ fontSize: '20px', fontWeight: 700, color: 'var(--forest)' }}>
                        {formatINR(p.price)}
                      </span>
                      <span style={{
                        fontSize: '10px', fontWeight: 600, letterSpacing: '0.08em',
                        textTransform: 'uppercase', color: 'var(--sage)',
                        background: '#EEF2E8', padding: '3px 8px', borderRadius: '4px'
                      }}>For Sale</span>
                    </div>
                  </div>
                </article>
            ...
            {paginated.length === 0 && (
              <div style={{ textAlign: 'center', padding: '80px 24px', color: 'var(--muted)' }}>
                ...
                <p style={{ fontSize: '16px' }}>No properties match your filters.</p>
              </div>
            )}
        ...
        {showDetail && selectedProp && (
          <div className="modal-overlay" onClick={closeDetail}>
            <div style={{
              ...
              width: '100%', maxWidth: '720px', margin: '16px'
            }} onClick={e => e.stopPropagation()}>
              <div style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '20px 24px', borderBottom: '1px solid var(--border)'
              }}>
                <h3 className="font-display" style={{ fontSize: '22px', fontWeight: 600, margin: 0 }}>
                  {selectedProp.title}
                </h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '24px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '12px' }}>
                    {selectedProp.amenities.slice(0, 4).map(t => (
                      <span key={t} style={{
                        ...
                        background: '#EEF2E8', color: 'var(--forest)',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <div className="font-display" style={{ fontSize: '28px', fontWeight: 700, color: 'var(--forest)' }}>
                    {formatINR(selectedProp.price)}
                  </div>
                  <p style={{ fontSize: '13px', color: 'var(--muted)', margin: 0 }}>
                    {selectedProp.type} · {selectedProp.beds} BHK · {selectedProp.area} sqft<br />
                    {selectedProp.city}, {selectedProp.state}
                  </p>
                  <p style={{ fontSize: '14px', lineHeight: 1.6, color: 'var(--charcoal)', margin: 0 }}>
                    {selectedProp.desc}
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: 'auto' }}>
                    <button onClick={openContact} className="btn-primary" style={{
                      padding: '12px', borderRadius: '10px', fontWeight: 600, fontSize: '14px'
                    }}>Contact Owner</button>
                    <button onClick={() => loadMap(selectedProp.city, selectedProp.state)} style={{
                      padding: '12px', borderRadius: '10px', fontWeight: 600, fontSize: '14px',
                      background: '#EEF2E8', color: 'var(--forest)', border: 'none', cursor: 'pointer',
        ...
        {showMap && (
          <div className="modal-overlay" onClick={closeMap}>
            <div style={{
              background: 'white', borderRadius: '20px',
              width: '100%', maxWidth: '900px', height: '75vh',
        ...
        {showFavorites && (
          <div className="modal-overlay" onClick={closeFavorites}>
            <div style={{
              ...
              overflow: 'hidden', display: 'flex', flexDirection: 'column', margin: '16px'
            }} onClick={e => e.stopPropagation()}>
              <div style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '20px 24px', borderBottom: '1px solid var(--border)'
                    ...
                    {favoriteProperties.map(p => (
                      <div key={p.id} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: '12px', color: 'var(--muted)', margin: '0 0 6px' }}>{p.city}, {p.state} · {p.area} sqft</p>
                          <p className="font-display" style={{ fontSize: '16px', fontWeight: 700, color: 'var(--forest)', margin: '0 0 8px' }}>{formatINR(p.price)}</p>
                          <button onClick={() => toggleFavorite(p.id)} style={{
                            background: 'none', border: 'none', color: '#e53935',
                            fontSize: '12px', cursor: 'pointer', fontWeight: 500, padding: 0,
